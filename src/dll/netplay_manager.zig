...
// each in_game index. The hypothesis was that early-frame states diverge
// between peers due to different absolute world_timer values at in_game entry.
//
// This was WRONG and caused the exact "desync fail to rollback and close"
// symptom the user reported: a misprediction at frame 0-7 was deferred, the
// divergence compounded through frames 1-7 with wrong predicted inputs, and
// by the time the deferred rollback fired at frame 8 it loaded an
// already-divergent state. The SyncHash check then caught the residual
// divergence and force-closed the match.
//
// CCCaster has NO early-frame gate — it rolls back to lcf_frame directly,
// re-simulating from the actual point of misprediction with corrected inputs.
// The state at frame 0 is deterministic because both peers apply the same
// synced RNG before in_game entry, and `frame = world_timer - start_world_time`
// is relative (so different absolute world_timer values don't matter). We
// match CCCaster's behavior here.
