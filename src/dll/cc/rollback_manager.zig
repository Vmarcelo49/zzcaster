...
const constants = @import("cc_constants.zig");
const mem_dump = @import("mem_dump.zig");
const IndexedFrame = @import("indexed_frame.zig").IndexedFrame;

// NetplayState is stored as a raw u8 (the enum int value) so this module
// doesn't depend on zzcaster's NetplayState enum. The adapter converts.
...
test "saveState + loadState round-trips a single region" {
    ...
    var orig_state: u8 = 4; // chara_select
    ...
    try expect(ok);
    try expectEqual(@as(u8, 8), orig_state);
